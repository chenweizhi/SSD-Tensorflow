    ...
    with tf.name_scope(scope, 'ssd_losses'):
        ...
        with tf.name_scope('cross_entropy_pos'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                  labels=gclasses)
            loss = tf.div(tf.reduce_sum(loss * fpmask), tf.cast(n_positives,tf.float32), name='value')
            tf.losses.add_loss(loss)

        with tf.name_scope('cross_entropy_neg'):
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=logits,
                                                                  labels=no_classes)
            loss = tf.div(tf.reduce_sum(loss * fnmask), tf.cast(n_neg,tf.float32), name='value')
            tf.losses.add_loss(loss)
        ...
        with tf.name_scope('localization'):
            ...
            loss = tf.div(tf.reduce_sum(loss * weights), tf.cast(n_positives,tf.float32), name='value')
            tf.losses.add_loss(loss)
