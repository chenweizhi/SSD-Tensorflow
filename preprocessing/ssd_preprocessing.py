...
def apply_with_random_selector(x, func, num_cases):
  ...
  return control_flow_ops.merge([
      func(control_flow_ops.switch(x, tf.equal(sel, case))[1], case) #when sel == case , return value (left, right) of switch left=none, right = real x
      for case in range(num_cases)])[0]                              #it is clear to see "Pass the real x only to one of the func calls."
...
def ssd_random_sample_patch(image, labels, bboxes, ratio_list=[0.1, 0.3, 0.5, 0.7, 0.9, 1.], name=None):
  ...
  def check_roi_center(width, height, labels, bboxes):
    with tf.name_scope('check_roi_center'):
      ...
      def condition(index, roi, mask):
        return tf.logical_or(tf.logical_and(tf.reduce_sum(tf.cast(mask, tf.int32)) < 1, #if one of the box in roi ,return
                                          tf.less(index, max_attempt)),
                            tf.less(index, 1))
  ...
  def check_roi_overlap(width, height, labels, bboxes, min_iou):
    with tf.name_scope('check_roi_overlap'):
      ...
      def condition(index, roi, mask_labels, mask_bboxes):
        return tf.logical_or(tf.logical_or(tf.logical_and(tf.reduce_sum(tf.cast(jaccard_with_anchors(roi, mask_bboxes) < min_iou, tf.int32)) > 0, #jaccard score must be bigger than min_iou
                                                        tf.less(index, max_attempt)),
                                          tf.less(index, 1)),
      ...
      return tf.cond(tf.greater(tf.shape(mask_labels)[0], 0), #if reutnr none, using default
  ...
  with tf.name_scope('ssd_random_sample_patch'):
    ...
    samples_min_iou = tf.multinomial(tf.log([[1. / len(ratio_list)] * len(ratio_list)]), 1) # probability
...
def ssd_random_expand(image, bboxes, ratio=2., name=None):
  with tf.name_scope('ssd_random_expand'):
    ...
    x = tf.random_uniform([], minval=0, maxval=canvas_width - width, dtype=tf.int32)# random to pad top and bottom ,left and right
...
def preprocess_for_train(image, labels, bboxes, out_shape, data_format='channels_first', scope='ssd_preprocessing_train', output_rgb=True):
  ...
  with tf.name_scope(scope, 'ssd_preprocessing_train', [image, labels, bboxes]):
    ...
    random_sample_image, labels, bboxes = ssd_random_sample_patch_wrapper(distort_image, labels, bboxes)#random select a patch and make sure bboxes is adaptive
